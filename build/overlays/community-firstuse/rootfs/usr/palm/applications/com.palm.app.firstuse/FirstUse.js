...
enyo.kind({
	...
	// webOS CE OOBE: finishing first use. Mark it done (writes ran-first-use) and
	// show a "Finishing Setup" screen — but do NOT reboot or close here. A device
	// reboot from firstuse kills the concurrent first-boot app-install, which
	// consumes staged ipks as it goes, so staged 1P apps would be lost. The
	// ce-finish-firstuse upstart job performs the single reboot into the full
	// launcher once app-install has finished. Every exit converges here: the Done
	// button (completeDoneTap), the community Skip links (wosaSkipSetup), and
	// firstUseComplete.
	// webOS CE OOBE: finishing first use. markFirstUseDone() writes ran-first-use
	// AND emits "first-use-finished", which powerd handles by rebooting the device
	// into the full launcher — so this is the single, correct end of first use.
	// By the time the user reaches here the first-boot app-install has finished
	// (its Preware bootstrap runs in a separate post-install job, NOT in-line, so
	// it never slows the install). Every exit converges here: Done
	// (completeDoneTap), the community Skip links (wosaSkipSetup), firstUseComplete.
	closeApp: function(){
