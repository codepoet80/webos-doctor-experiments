enyo.kind({
	...
	components: [
		...
		{name: "groupButton", layoutKind: "HFlexLayout", defaultKind: "Button", components: [
			...
			{flex: 1, layoutKind: "VFlexLayout", pack: "center", caption: $L("Done"), onmousedown: "unlock",/*onclick: "unlock",*/ className: "pin-menu-button"}
		]},
		{className: "firstuse-bottom-corners"},		
		{name: "groupControl", layoutKind: "HFlexLayout", className: "firstuse-banner-container", components: [ 
	...
	devicePinVerifyResponse: function(inSender, response){
		enyo.log("------------devicePinVerifyResponse: ",enyo.json.stringify(response)); 
		if (response.returnValue) {
			enyo.log("time to get response: " + (Date.now() - this._timer))
	...
	onSetPin: function(inSender, response) {
		enyo.log("onSetPin"+enyo.json.stringify(response)); //todo: take me out later		
		this.reset();
		this.$.sysmanagerService.call({
			state: 'unlock'
		},{
...
enyo.log(enyo.json.stringify(response)); //temp log to track a policy issue		
