...
enyo.kind({
	...
	create: function() {
		...
		if (deviceInfo && deviceInfo.keyboardType) {
			...
			if (deviceInfo.keyboardType === 'QWERTZ' || deviceInfo.keyboardType === 'QWERTZ_DE') {
				this.qwertzKey = true; 
			}
		}
	},
	...
	translateKey:function(c){
		...
		switch (c){
			...
			case 'z':
			case 'Z':
				return (this.azertyKey || this.qwertzKey) ? "" : '*';
			case 'y':
			case 'Y':
				return this.qwertzKey ? '*' : "";
			case 'b':
			case 'B':
				return '#';
			case 'w': case 'W':
	...
	ignoreFirstDtmfTimeout: function() {
		enyo.log("ignoreFirstDtmfTimeout");
	...
	sendContinuousDTMF: function(tone){
		if (this.continuousDTMFInProgress === undefined || this.continuousDTMFInProgress === false) {
			...
			//enyo.log( "sendContinuousDTMF : " + tone);
			if (this.useFeedback && this.feedbackOnly) {
				 this.$.playDialTone.call({
				 		"id": String(tone),
				 		"oneshot": false,
...
enyo.kind({
  ...
  feedbackOnly: true, 
	...
	components: [
		{kind: "HFlexBox", flex: 1, defaultKind: "DialpadButton", components: [
			{number: "*", flex: 1, className: "dialpadbutton-left asterisk", numberFontSize: "52px"},
			{number: "0", subtext: "+", flex: 1, className: "dialpadbutton-middle"},
			{number: "#", flex: 1, className: "dialpadbutton-right"}
		]}
	]
...
enyo.kind({
	...
	components: [
		{kind: "HFlexBox", flex: 1, defaultKind: "DialpadButton", components: [
			...
			{kind: "IconButton", flex: 1, value: "backspace", layoutKind: "VFlexLayout", pack: "center", className: "dialpadbutton-bottom-right dialpad-backspace", icon: "images/dialpad-backspace.png", onclick: "handleBackspace", onmousehold: "handleClear"}
		]}
	],
	create: function(){
		this.inherited(arguments);
