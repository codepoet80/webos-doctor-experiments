...
enyo.kind({
	...
	gotFailure: function(inSender, inResponse, inRequest) {
		enyo.warn("GAL search failed, call:", this.id, "accountId", inRequest.params.accountId, "error:", inResponse.errorText);
		this.inflight--;
	...
	fetchContacts: function() {
		...
		for (var i=0, a; a=accounts[i]; i++) {
			...
			enyo.log("GAL search, call :", this.id, "accountId:", a._id);
		}
	},
	...
	gotResults: function(inSender, inResponse, inRequest) {
		this.inflight--;
		enyo.log("GAL search successful, call:", this.id, "num results:", inResponse.results.length, "accountId:", inRequest.params.accountId);
