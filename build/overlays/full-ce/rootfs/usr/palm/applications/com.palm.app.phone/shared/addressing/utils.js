...
enyo.addressing.appendContactDisplayAddresses = function(inPerson, inTypes, inImTypes, inSortFilter) {
	...
	for (var i=0, t, a; t=inTypes[i]; i++) {
		...
		if (a) {
			...
			if (inSortFilter) {
				enyo.addressing.sortAddresses(inPerson, inSortFilter);
			}
		}
	}
}

enyo.addressing.longestSubstring = function (inString, inFilter) {
	if (!inString || !inFilter) {
		return 0;
	}
	var longest = 0;
	var previous = [], current = [];
	// dynamic programming longest common substring with O(n) storage
	for (var i = 0, l = inString.length; i < l; i++) {
		for (var j = 0, ll = inFilter.length; j < ll; j++) {
			if (inString[i] === inFilter[j]) {
...
enyo.addressing.sortAddresses = function (inPerson, inSortFilter) {
	var ipda = inPerson.displayAddresses;
	if (ipda.length > 1) {
		var matches = {};
		ipda.forEach(function(a) {
			var va = a.formattedValue
			matches[va] = enyo.addressing.longestSubstring(va, inSortFilter)
		});
		ipda.sort(function(a, b) {
			var va = a.formattedValue, vb = b.formattedValue;
			return matches[vb] - matches[va];
		});
	}
}
