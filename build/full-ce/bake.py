...
def externalise_job_scripts():
    ...
    Semantics are preserved exactly: bodies still run under `sh -e` (guards
    must stay `[ ! -d x ] || cmd`, never `[ -f x ] && exit 0`), and the job
    keeps its script/end script shape so nothing about its lifecycle changes.
    """
